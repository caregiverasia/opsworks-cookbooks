...
node[:deploy].each do |application, deploy|
  ...
  default[:deploy][application][:shared_bundler_home] = node[:opsworks][:rails][:shared_bundler_home]
  ...
  if node[:deploy][application][:auto_bundle_on_deploy]
    default[:deploy][application][:migrate_command] = ". /etc/profile; if [ -f Gemfile ]; then echo 'OpsWorks: Gemfile found - running migration with bundle exec' && bundle exec #{node[:deploy][application][:rake]} db:migrate; else echo 'OpsWorks: no Gemfile - running plain migrations' && #{node[:deploy][application][:rake]} db:migrate; fi"
  else
